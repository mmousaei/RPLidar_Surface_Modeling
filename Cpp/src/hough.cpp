...
std::vector<double> Hough::initializeHough(std::vector<std::pair<double, double>> *cartesian)
{
  ...
  int d_ind = 0;
  double a_min = 1000, a_max = 0, b_min = 1000, b_max = 0;
  ...
  for(int i = 0; i < cartesian->size(); i++)
  {
    ...
    {
      if(cartesian->at(i).first < a_min) a_min = cartesian->at(i).first;
      if(cartesian->at(i).first > a_max && cartesian->at(i).first < 1) a_max = cartesian->at(i).first;
      if(cartesian->at(i).second < b_min) b_min = cartesian->at(i).second;
      if(cartesian->at(i).second > b_max && cartesian->at(i).second < 1) b_max = cartesian->at(i).second;
  ...
  int asize = (a_max - a_min) / a_inc + 1;
  int bsize = (b_max - b_min) / b_inc + 1;
  int rsize = (r_max - r_min) / r_inc + 1;
  //
  //
  // // std::vector<std::vector<std::vector<int>>> houghArray(asize, std::vector<std::vector<int>>(bsize, std::vector<int>(rsize)));
  //

  static int houghArray[1000][1000][100];
  ...
  for(int i = 0; i < vsize; i++)
  {
    ...
    {
      for(double r = r_min; r < r_max; r += r_inc)
      {
        for(int theta = 0; theta < 360; theta++)
        {
          if(!std::isinf(cartesian->at(i).first))
          {
            a_int = (int) cartesian->at(i).first + r*cos((double) theta * 3.1415 / 180);
            b_int = (int) cartesian->at(i).second + r*sin((double) theta * 3.1415 / 180);
            r_int = r/r_inc - r_min;
            houghArray[i][j][r_int]++;
            if(houghArray[i][j][r_int] > MaxH)
            {
  ...
  res.push_back(a_hough);
  res.push_back(b_hough);
  res.push_back(r_hough);
...
void Hough::convertToMat(std::vector<std::pair<double, double>> *cartesian)
{
  ...
  //              Matrix.rows/16,  // change this value to detect circles with different distances to each other
